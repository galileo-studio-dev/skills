...
Step 7 and the error-recovery guidance above run commands that change real
infrastructure or shared state: `terraform apply`, `terraform destroy`,
`terraform import`, `terraform refresh`, `terraform state mv|rm|push`,
`terraform force-unlock`, `terraform workspace delete`, and backend
migrations. Before any of them, show the exact command and the
`terraform plan` output it is based on, and wait for explicit approval.
Never run them against a production workspace without a reviewed plan, and
never use `-lock=false`. `terraform fmt`, `terraform validate`,
`terraform plan`, and `tflint` may run without approval.

Do not install Terraform, `tflint`, or providers outside `terraform init`
without approval. Never print, log, or commit state files, `.tfvars` files
containing secrets, or provider credentials; redact them from anything
shared back to the user.

## Credits

Adapted from [`terraform-engineer`](https://github.com/Jeffallan/claude-skills/tree/882ef55e377dbf9a4dbe496bb41ac6ccd0e555cf/skills/terraform-engineer) by [Jeffallan](https://github.com/Jeffallan). This vendored copy adds this Credits section and company approval notes; the upstream MIT license is included in this skill directory.
