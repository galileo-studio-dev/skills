...
This company-approved copy may run CodeQL, build commands, dependency restore
commands, and SARIF processing against a target codebase. Before running a
workflow that installs dependencies, deletes build artifacts, or changes the
target workspace, present the exact commands and wait for explicit user
approval.

Generated CodeQL databases, diagnostics, extensions, logs, suites, and results
must stay under the resolved `OUTPUT_DIR` unless the user explicitly requests a
different path. Do not install CodeQL or global package-manager tooling
automatically. Redact secrets, tokens, private URLs, and account identifiers
from reports and logs shared back to users.

## Credits

Adapted from [`codeql`](https://github.com/trailofbits/skills/tree/cfe5d7b1619e47fb5b38b7e2561dad7e5f1e89af/plugins/static-analysis/skills/codeql) in the Trail of Bits [`static-analysis`](https://github.com/trailofbits/skills/tree/cfe5d7b1619e47fb5b38b7e2561dad7e5f1e89af/plugins/static-analysis) plugin by Axel Mierczuk, Paweł Płatek, and [Trail of Bits](https://github.com/trailofbits). This vendored copy adds this Credits section and company approval notes; the upstream CC-BY-SA-4.0 license is included in this skill directory.
