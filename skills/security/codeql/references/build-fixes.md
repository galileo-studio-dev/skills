...
Company-approved copy: before running any fix that deletes build artifacts,
installs dependencies, refreshes dependency caches, or changes target project
state, present the exact commands and wait for explicit user approval. Never
use `sudo`, global package-manager installs, or cleanup paths that are not
resolved inside the target workspace or CodeQL output directory.
