...
## Company Approval Notes

This company-approved copy is intended for security scanning only. Before
running a scan, present the exact target path, output directory, scan mode,
engine mode, and rulesets, then wait for explicit user approval.

All generated files must stay under the resolved `OUTPUT_DIR`. Do not install
Semgrep or other tooling automatically. Do not report raw secret values found
by `p/secrets`; redact credential material and include only enough context for
the team to locate and rotate the secret.

The upstream plugin-level scanner agent is included in this vendored package as
`agents/semgrep-scanner.md` for provenance. If the runtime does not expose that
subagent type, execute the scanner prompt directly with the available approved
tooling.

## Credits

Adapted from [`semgrep`](https://github.com/trailofbits/skills/tree/cfe5d7b1619e47fb5b38b7e2561dad7e5f1e89af/plugins/static-analysis/skills/semgrep) in the Trail of Bits [`static-analysis`](https://github.com/trailofbits/skills/tree/cfe5d7b1619e47fb5b38b7e2561dad7e5f1e89af/plugins/static-analysis) plugin by Axel Mierczuk, Paweł Płatek, and [Trail of Bits](https://github.com/trailofbits). This vendored copy adds this Credits section and company approval notes; the upstream CC-BY-SA-4.0 license is included in this skill directory.
