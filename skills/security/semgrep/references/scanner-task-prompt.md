...
```bash
test -n "[OUTPUT_DIR]" && test "[OUTPUT_DIR]" != "/" && test "[OUTPUT_DIR]" != "." && rm -rf "[OUTPUT_DIR]/repos"
```
...
- After all scans complete, delete only [OUTPUT_DIR]/repos/ after confirming [OUTPUT_DIR] is an absolute, resolved output path and not `/` or `.`
...
### Clean up cloned repos:
```bash
test -n "/path/to/static_analysis_semgrep_1" && test "/path/to/static_analysis_semgrep_1" != "/" && test "/path/to/static_analysis_semgrep_1" != "." && rm -rf "/path/to/static_analysis_semgrep_1/repos"
```
...
- Delete only the repos/ subdirectory under the resolved output directory after scanning
