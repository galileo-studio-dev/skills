...
- The Reviewer's read-only guard covers `delivery:reviewer` only. The sub-reviewers it dispatches are `general-purpose` agents, read-only by instruction, not by hook.
