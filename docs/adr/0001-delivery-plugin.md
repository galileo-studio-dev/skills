...
Cost policy (added after the pilot and the September 2026 cost research): every role and the orchestrator run at `medium` effort; a `worker-high` agent exists only for fix cycles; sessions run on Claude Opus 5.5, which the Controller and the Reviewer inherit, and Workers run on Sonnet for Prototype work via the Agent tool's `model` override; briefs carry line ranges and roles read by range; handoffs and reviews are size-contracted; the evals report cost, cache-read share and cost per verified slice (`evals/lib/usage.py`), which is the metric changes are judged by. The pilot's ledger drove this: 87% of its tokens were cache reads, the Controller's context × turns was 42% of the run, nested sub-reviewers 25%, and four whole-file reads of a 700-line module happened where line ranges were known. Opus 5.5 (22 September 2026) settled the model question: cache reads at $0.20/MTok match Sonnet 5 and undercut Fable 5.1 ($0.25) and Opus 5 ($0.50) at Fable-level quality, so the pilot's own ledger recomputed at published rates costs about $4.9 instead of $11.4.
...
- Model routing measured on the fixture (2026-09-23, Claude Code 2.1.280): an Opus 5.5 session cost $1.72 ($0.86 per slice, 88% cache reads) and its review caught an uncovered requirement (R3) that the all-Sonnet session ($0.98, $0.49 per slice) recorded only as a Minor note; the fix cycle it triggered is why that run has three commits, which is what exposed the eval checker's fixed commit count. Recomputed at published rates, the pilot's Fable 5.1 ledger would cost about $4.9 on Opus 5.5 against $11.4.
